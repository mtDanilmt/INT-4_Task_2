...
Для создания HTTP-сервера используется встроенная библиотека `http.server`. Можно было бы сделать тоже самое с помощью фреймворков Flask и Django. Но, по моему мнению, использование их в реалиах не больших задач является нецелесообразным. (При этом же ещё необходимо будет скачивать их, что потребует дополнительного места на диске).
